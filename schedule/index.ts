...
async function writeManagedCrontab(managed: string[]) {
	...
	const begin = lines.indexOf(CRON_BEGIN);
	const end = lines.indexOf(CRON_END);
	const head = begin >= 0 ? lines.slice(0, begin) : lines.filter((line) => line.trim());
	const tail = begin >= 0 && end > begin ? lines.slice(end + 1).filter((line) => line.trim()) : [];
	const next = [...head.filter((line) => line.trim()), CRON_BEGIN, ...managed, CRON_END, ...tail.filter((line) => line.trim())].join("\n") + "\n";
...
function createRunnerScript(entry: ScheduleEntry): string {
	const cleanup = entry.backend === "cron_fallback"
		? `
...
set +e
pi --print${entry.model ? ` --model ${shellEscape(entry.model)}` : ""} ${shellEscape(entry.task)} > "$RESULT_PATH" 2>&1
EXIT_CODE=$?
set -e
...
async function scheduleOneShot(entry: ScheduleEntry): Promise<ScheduleEntry> {
	if (await commandExists("at")) {
		...
		return { ...entry, backend: "at", atJobId: jobId };
	}
	...
	const cronLine = `${cronSpecForDate(new Date(entry.runAt))} ${shellEscape(entry.scriptPath)} # ${cronId}`;
	await installCronLine(cronId, cronLine);
	return { ...entry, backend: "cron_fallback", cronId };
}
...
function formatEntry(entry: ScheduleEntry, state?: ScheduleRunState) {
	const status = entry.cancelledAt
		? `cancelled at ${entry.cancelledAt}`
		: state?.endedAt
			? `completed (${state.exitCode ?? 0}) at ${state.endedAt}`
			: state?.startedAt
				? `running since ${state.startedAt}`
				: `scheduled for ${entry.runAt}`;
		...
		handler: async (args, ctx) => {
			...
			if (raw.startsWith("logs ")) {
				...
				if (!state?.resultPath || !existsSync(state.resultPath)) {
					ctx.ui.notify(`No result yet for ${id}`, "warning");
					return;
				}
			...
			const match = raw.match(/^(in\s+\d+\s+(?:minute|minutes|hour|hours|day|days)|\d{4}-\d{2}-\d{2}[^\s]*)\s+([\s\S]+)$/i);
			if (!match) {
				ctx.ui.notify("Usage: /schedule <time> <task> (examples: /schedule in 10 minutes ... or /schedule 2026-04-21T09:00:00 ...)", "error");
				return;
			}
			const when = match[1].trim();
			const task = match[2].trim();
			const runAt = parseWhenSpec(when);
			if (!runAt) {
				ctx.ui.notify(`Could not parse time: ${when}`, "error");
				return;
			}
			if (runAt.getTime() <= Date.now()) {
				ctx.ui.notify("Scheduled time must be in the future.", "error");
				return;
			}
			const id = createId();
			await ensureStore(store);
			...
			await writeFile(scriptPath, createRunnerScript(entry), "utf8");
			await chmod(scriptPath, 0o755);
			entry = await scheduleOneShot(entry);
			entries.push(entry);
			await writeIndex(store, entries);
			ctx.ui.notify(`Scheduled ${id} for ${entry.runAt} using ${entry.backend}`, "info");
		},
	});
}
